...
exports.createSpace = async (req, res) => {
  ...
  try {
    ...
    // Extra check: does the file exist right after upload?
    const fs = require('fs');
    if (resumePath) {
      fs.access(resumePath, fs.constants.F_OK, (err) => {
        if (err) {
          console.error('File does NOT exist immediately after upload:', resumePath);
        } else {
          console.log('File exists after upload:', resumePath);
        }
      });
    }
